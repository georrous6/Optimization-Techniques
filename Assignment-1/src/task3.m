...
%% 1. Fibonacci Search: Evaluations vs tolerance
...
for i = 1:length(L)
    l = L(i);
    [~,~,K(1, i)] = fibonacci_method(a, b, l, e, @f1);
    [~,~,K(2, i)] = fibonacci_method(a, b, l, e, @f2);
    [~,~,K(3, i)] = fibonacci_method(a, b, l, e, @f3);
end

figure(1);
plot(L, K(1,:) + 1, 'g:', 'LineWidth', 2, 'DisplayName', 'f1');
hold on;
plot(L, K(2,:) + 1, 'b--', 'LineWidth', 2, 'DisplayName', 'f2');
plot(L, K(3,:) + 1, 'r-.', 'LineWidth', 2, 'DisplayName', 'f3');
legend show;
xlabel('Tolerance');
ylabel('Evaluations of objective function');
title('Fibonacci Search: Evaluations vs Tolerance');
exportgraphics(gcf, fullfile(outputDir, 'task3_plot1.pdf'));
...
figure(2);
sgtitle('Fibonacci Search: Bounds Tracking Across Iterations');

% Function 1
subplot(3, 1, 1);
for i = 1:length(L)
    l = L(i);
    [A,B,k] = fibonacci_method(a, b, l, e, @f1);
    K = 1:k;
...
for i = 1:length(L)
    l = L(i);
    [A,B,k] = fibonacci_method(a, b, l, e, @f2);
    K = 1:k;
    hA = plot(K, A, 'Color', colors(i), 'LineWidth', 2, 'DisplayName', sprintf('l=%.3f', l));
    hold on;
    plot(K, B, 'Color', colors(i), 'LineWidth', 2);
...
for i = 1:length(L)
    l = L(i);
    [A,B,k] = fibonacci_method(a, b, l, e, @f3);
    K = 1:k;
    hA = plot(K, A, 'Color', colors(i), 'LineWidth', 2, 'DisplayName', sprintf('l=%.3f', l));
    hold on;
    plot(K, B, 'Color', colors(i), 'LineWidth', 2);
...
xlabel('Iterations');
exportgraphics(gcf, fullfile(outputDir, 'task3_plot2.pdf'));
